...
/* ==========================================================
   ...
   C'est une "empreinte digitale" d'un message.
   Si le message change d'un seul caractère, le hash change complètement.
   On ne peut pas retrouver le message original à partir du hash.
   ========================================================== */
...
/* ==========================================================
   ...
   Qu'est-ce qu'AES ?
   C'est un algorithme de chiffrement symétrique : la même clé
   sert à chiffrer ET à déchiffrer. Très rapide et très sécurisé.
   Mode CBC = Cipher Block Chaining (chaînage de blocs).
   ========================================================== */
...
/* ==========================================================
   ...
   Note importante pour le débutant :
   Le vrai RSA nécessite une bibliothèque complexe non disponible
   nativement dans le navigateur. On simule ici son comportement
   avec une fonction de hachage simple pour la démonstration.
   
   Qu'est-ce que RSA ?
   C'est un algorithme asymétrique : on utilise une clé PUBLIQUE
   pour chiffrer, et une clé PRIVÉE pour déchiffrer.
   Tout le monde connaît la clé publique, mais seul le propriétaire
   connaît la clé privée.
   ========================================================== */
...
async function sendMessage() {
  ...
  // Ajout à la timeline
  addEvent(isAttack
    ? '☠️ <strong style="color:var(--danger)">ATTAQUE</strong> : message intercepté et modifié !'
    : '<strong>Transmission</strong> sécurisée vers Koffi'
  );

  if (isAttack) {
    log('☠️ ATTAQUE simulée : message et signature modifiés en transit !', 'warn');
  } else {
...
/* ==========================================================
   ...
   Principe : on décale chaque lettre de l'alphabet d'un certain nombre
   de positions. Exemple avec un décalage de 3 :
   A → D, B → E, Z → C (on "tourne" en boucle)
   
   Vulnérabilité : il n'y a que 25 décalages possibles.
   Un ordinateur peut tous les tester en moins d'une seconde !
   ========================================================== */
...
/* ==========================================================
   ...
   Principe : comme César, mais avec un mot-clé de plusieurs lettres.
   Chaque lettre du message est décalée par la lettre correspondante du mot-clé.
   Exemple avec la clé "KOFFI" :
   - 1ère lettre → décalée par K (10 positions)
   - 2ème lettre → décalée par O (14 positions)
   - 3ème lettre → décalée par F (5 positions)
   - etc.
   
   Vulnérabilité : l'analyse de Kasiski permet de trouver la longueur
   de la clé en cherchant des répétitions dans le texte chiffré.
   ========================================================== */
